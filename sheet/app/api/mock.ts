...
export function mockFetch({
  ...
  pageSize,
  value,
}: AccountDataRequestInput): Promise<AccountDataResult> {
  const random = Math.random();
  if (random >= 0.3) {
    return onSuccess({ page, pageSize, value });
  } else {
    return onError();
  }
}
...
function onSuccess({
  ...
  return new Promise((resolve) => {
    ...
    const isIdQuery = Number(value);
    const filteredResults = isEmptyQuery
      ? mockData
      : mockData.filter(({ id, name }) => {
          if (isIdQuery) {
            return containsDigit(id, value);
          } else {
    ...
    setTimeout(() => {
      resolve({
        data: filteredResults.slice((page - 1) * pageSize, page * pageSize),
